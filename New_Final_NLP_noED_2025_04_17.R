...
constant    = rep(-0.16,length(subsets))   
...
log_liks_stage1 = function(R.i, Y.i, label.i) {
  trt_int   = R.i[,c(3:6)]
...
posterior_gamma_corrected = function(logprob_data_given_models_lp_nlp_total) lapply(1:length(logprob_data_given_models_lp_nlp_total), function(iter){
  label = iter
  posterior_mode_vec =  logprob_data_given_models_lp_nlp_total[[label]]$posterior_mode
...
data_stage_12_full_list = sapply(1:n_stage1_trials, function(i){
  ...
  logliks= sapply(logprob_data_given_models_lp_nlp_total, function(V)V$logprob_data_given_model)
  which.max(logliks)
  ...
  posterior_mode_list = posterior_gamma_corrected(logprob_data_given_models_lp_nlp_total)
  
  ## calculating u1 
  unweighted_u1 = sapply(1:length(logprob_data_given_models_lp_nlp_total), function(iter){ 
  ...
  Theta_i_pred_list =lapply(1:length(sampled_models), function(iter){
    ...
    importance_samples =  logprob_data_given_models_lp_nlp_total[[label]]$importance_samples
    ...
    log_importance_weights = logprob_data_given_models_lp_nlp_total[[label]]$log_importance_weights
...
saveRDS(data_stage_12_full_list, paste0("NLP_noED_new_dstar_mat_Sc",Job_mat[a,"Sc"],"batch",Job_mat[a,"Job"],"_c_",paste0(gsub("\\.", "point", as.character(constant[1]))),".rds"))
